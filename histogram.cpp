...
vector<Mat>* histogram1D (Mat imgSrc) {	
	...
	Mat histImage2( hist_h, hist_w, CV_8UC3, Scalar(255,255,255) );
	 ...
	 /// Normalize (Aufgabe 5): Normalisiert in Relation zur maximalen Pixelanzahl
 	normalize(b_hist, b_hist_org, histImage.rows, 0, NORM_L1, -1, Mat() );
  	normalize(g_hist, g_hist_org, histImage.rows, 0, NORM_L1, -1, Mat() );
  	normalize(r_hist, r_hist_org, histImage.rows, 0, NORM_L1, -1, Mat() );

	 /// Normalize (AUFGABE 4): Normalisiert die Werte in Relation zum höchsten Wert im Histogram (siehe Funktion minMaxLoc)
	...
	// Attention: (0,0) begins at top left corner, thats why we calculate hist_h - vector[i]
	for (int i = 0; i < binSize*3; i = i += 3) {
		rectangle (histImage2, Point (bin_w*i, hist_h), Point (bin_w*(i+1), hist_h - b_hist_org.at<float>(i/3)), Scalar(255, 0, 0), CV_FILLED);
		rectangle (histImage2, Point (bin_w*(i+1), hist_h), Point (bin_w*(i+1+1), hist_h - g_hist_org.at<float>(i/3)), Scalar(0, 255, 0), CV_FILLED);
		rectangle (histImage2, Point (bin_w*(i+2), hist_h), Point (bin_w*(i+2+1), hist_h - r_hist_org.at<float>(i/3)), Scalar(0, 0, 255), CV_FILLED);
	}

 	// Display
 	namedWindow("Histogram A4", CV_WINDOW_AUTOSIZE );
	namedWindow("Histogram A5", CV_WINDOW_AUTOSIZE );
 	imshow("Histogram A4", histImage );
 	imshow("Histogram A5", histImage2 );
